...
**面向大文档仓库的增强方案（planned）**：

当单项目 wiki 页面超过 50 篇时，`ProjectDetail` 不应继续只显示单一平铺列表，而应升级为 `Project Knowledge Explorer`，采用“树形导航 + 规范分组 + 搜索/图谱”的混合结构。

| 方案 | 优点 | 问题 | 结论 |
|------|------|------|------|
| 纯平铺列表 | 实现简单 | 100+ 文档时扫描成本过高 | 不适合 Synapse 这类项目 |
| 纯目录树 | 与仓库结构一致，可预测 | 目录结构常常偏工程实现，不等于知识结构 | 应作为基础视图，不应是唯一视图 |
| 纯 LLM/Agent 重组目录 | 可以生成更“像文档站”的分组 | 结构不稳定、可能漂移、可能误分类 | 不应作为唯一真源 |
| 混合方案 | 稳定、可解释、可逐步增强 | 需要多一层索引产物 | 推荐 |

**推荐交互结构**：
- 顶部保留现有项目统计卡。
- Wiki 区切换为四种视图：`List` / `Tree` / `Domain` / `Graph`。
- `Tree` 视图基于 `source_path` 确定性生成，作为稳定真源。
- `Domain` 视图基于导入阶段生成的规范导航索引，作为更适合人阅读的知识分组。
- `Graph` 继续保留为跨主题探索入口，而不是主导航。

**Tree 视图规则**：
- 根目录核心文档固定归入 `Root Docs`。
- `docs/**` 按真实目录展开，如 `architecture / workflows / ops / plans`。
- 非 `docs/` 路径的补充知识源单独放入 `Code-Adjacent Docs` 或 `Other Sources`，避免污染主文档树。
- 每个节点显示文档数、最近更新时间、折叠状态，默认折叠深层目录。

**Domain 视图规则**：
- 由导入阶段生成稳定的“规范分组”，例如 `Architecture`、`Workflow`、`Ops`、`Plans`、`Integrations`。
- 分组不是自由生成目录，而是带 `source_path` 映射关系的索引层。
- 同一文档允许保留唯一物理路径，但可在索引层拥有别名入口。

**LLM / Agent 的合适职责**：
- 可以用于为一批文档生成更适合阅读的分组名、摘要、别名、排序建议。
- 不直接改写物理树结构，不替代 `source_path` 真源。
- 每个建议分组都必须回链到真实源文件列表，且允许人工覆盖。

**建议的数据产物（planned）**：
- `source_path`：真实物理路径，现有字段继续保留。
- `nav_path`：前端树形导航路径，例如 `docs/architecture/workflows`。
- `source_group`：规范分组，例如 `Architecture`。
- `document_role`：如 `root-doc`、`reference`、`plan`、`ops`。
- `project knowledge index`：项目级导航索引，供 Project Detail 渲染 `Tree` / `Domain` 双视图。

**UI 结果预期**：
- 进入 `http://localhost:10000/projects/:id` 后，默认先看到摘要视图，而不是直接滚动 100+ 卡片。
- 用户可先从树节点筛选，再看叶子文档列表。
- 当项目文档继续增长时，页面复杂度增长速度低于文档数增长速度。
...
**功能点**：
1. **任务组列表**：name / project / cron / status / last_run / next_run / last_result
2. **筛选与排障**：按 project / status / last_result / failed_only 过滤
3. **创建任务组**：固定内含 `docs/profile/plan`
4. **任务组详情**：最近 200 次执行历史、step 结果、workflow/checks 入口
5. **快捷操作**：编辑 / 启停 / 立即执行 / 删除

**后端实现**：自维护 Scheduler runtime；任务组持久化到 `memory/scheduler_tasks.json`，执行 batch 写入 `memory/scheduler_runs.jsonl`
...
/wiki/graph              → Knowledge Graph（Schema / Explore / Table）
...
**定位**：多视图知识图谱浏览器，先看 schema，再做局部 explore，最后用 table 精确筛选。
...
**交互设计**：
- `Schema`：项目 x 类型矩阵 + 关系概览，作为默认总览
- `Explore`：只展开单个焦点节点的 `1-hop / 2-hop` 邻居，避免全量图不可读
- `Table`：按项目、类型、关系和搜索词筛选，支持跳回 Explore 或 Wiki
- `?node=` 深链自动落到 `Explore`
- 默认不在缩放很小的状态下强制显示全部标签
...
| `KnowledgeGraph` | Schema / Explore / Table 多视图浏览 | /wiki/graph |
...
| `/api/projects/:id/wiki-nav` | GET | 单项目导航索引（`list/tree/domain` 三种模式）|
...
| `/api/scheduler/task-groups` | GET | 任务组列表 |
| `/api/scheduler/task-groups` | POST | 创建任务组 |
| `/api/scheduler/task-groups/:id` | GET | 任务组详情 |
| `/api/scheduler/task-groups/:id` | PUT | 编辑任务组 |
| `/api/scheduler/task-groups/:id/pause` | POST | 暂停任务组 |
| `/api/scheduler/task-groups/:id/resume` | POST | 恢复任务组 |
| `/api/scheduler/task-groups/:id/run` | POST | 立即执行任务组 |
| `/api/scheduler/task-groups/:id/runs` | GET | 任务组执行历史 |
...
| Knowledge Graph：Schema / Explore / Table 多视图浏览 | KnowledgeGraph.tsx |
| Lint Report：分组问题列表 + 快捷修复入口 | LintReport.tsx |
| Ingest 表单 + 摄入日志 | Ingest.tsx |
| 后端新增 API：wiki/graph + backlinks | api.py |

**验收**：Wiki 全部页面联通，Knowledge Graph 默认进入 Schema，Explore / Table 可切换，Ingest 成功写入。
...
| 风险 | 缓解 |
|------|------|
| Scheduler runtime 状态在进程重启后丢失 | 持久化任务组到 `memory/scheduler_tasks.json`，执行批次写入 `memory/scheduler_runs.jsonl` |
| 节点过多（>500 节点）导致全量图不可读 | 默认进入 Schema，Explore 只画局部邻居，Table 承担精确筛选 |
...
| 文档 | 与本文关系 |
|------|-----------|
| `01-tech-stack.md` | 本文沿用其技术决策，并把 Scheduler runtime 的前端承载方式细化到页面级 |
