...
### `amem enable . --full --ingest-wiki [--wiki-limit <n>]`

在 `--full` 的基础上继续自动发现并导入项目 Markdown 知识源到共享 wiki。
...
如果你想让 Agents-Memory 自动发现整个项目的 Markdown 知识源，不需要逐个文件执行 `wiki-ingest`，优先使用：

```bash
amem bootstrap . --ingest-wiki
```

或在前端 `http://localhost:10000/wiki/ingest` 里使用“项目自动接入”表单。
