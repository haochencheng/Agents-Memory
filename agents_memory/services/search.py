...
def build_fts_index(ctx: AppContext, force: bool = False) -> int:
    ...
    if not force:
        existing_count = _fts_doc_count(conn)
        if existing_count == len(all_files):
            conn.close()
    ...
    try:
        ...
        if ctx.vector_dir.exists():
            ...
            if "errors" in db.table_names():
                ...
                vec_rows = db.open_table("errors").search(q_vec).limit(limit * 2).to_list()
                for row in vec_rows:
                    distance = row.get("_distance", 1.0)
                    similarity = max(0.0, 1.0 - float(distance))
                    vector_by_id[row["id"]] = similarity
    except Exception:
        pass  # vector search unavailable, use FTS only

    # --- Merge ---
    all_ids = set(fts_by_id) | set(vector_by_id)
    merged: list[dict[str, Any]] = []
    for doc_id in all_ids:
        fts_score = fts_by_id.get(doc_id, {}).get("fts_score", 0.0)
        vec_score = vector_by_id.get(doc_id, 0.0)

        # If only one source contributed, use full weight for that source
...
def cmd_hybrid_search(ctx: AppContext, args: list[str]) -> int:
    ...
    mode = "FTS-only" if fts_only else "Hybrid FTS+Vector"
    print(f"\n{mode} 搜索结果: '{query}'  ({len(results)} 条)\n")
