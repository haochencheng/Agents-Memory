...
1. Before any substantial analysis or edits, call `memory_get_onboarding_state(project_root=".")` if the `agents-memory` MCP server is available.
   If the MCP tool is unavailable, read `.agents-memory/onboarding-state.json` directly if it exists.
