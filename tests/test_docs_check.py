...
def _write_open_source_files(root: Path, *, include_collaboration: bool = True) -> None:
    _write_text(root / "CHANGELOG.md", "# Changelog\n\n## Unreleased\n\n### Added\n- Demo entry\n")
    ...
    _write_raw_text(root / "SECURITY.md", "# Security Policy\n")
    _write_raw_text(root / "SUPPORT.md", "# Support\n")
...
class DocsCheckTests(unittest.TestCase):
    ...
    def test_collect_docs_check_findings_flags_missing_repository_health_entrypoints(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            _write_text(root / "README.md", _healthy_readme())
            _write_text(root / "docs" / "README.md", "- [Getting Started](getting-started.md)\n")
    ...
    def test_collect_docs_check_findings_flags_missing_release_flow_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            _write_text(root / "README.md", _healthy_readme())
            _write_text(root / "docs" / "README.md", "- [Getting Started](getting-started.md)\n")
    ...
    def test_collect_docs_check_findings_flags_missing_release_checklist_steps(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            _write_text(root / "README.md", _healthy_readme())
            _write_text(root / "docs" / "README.md", "- [Getting Started](getting-started.md)\n")
            ...
            self.assertTrue(any(f.status == "FAIL" and f.key == "open_source_release" for f in findings))
    ...
    def test_collect_docs_check_findings_flags_missing_split_ci_jobs(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            _write_text(root / "README.md", _healthy_readme())
            _write_text(root / "docs" / "README.md", "- [Getting Started](getting-started.md)\n")
            ...
            _write_text(
                ...
                "\n".join(
                    [
                        ...
                        "python3 scripts/memory.py close-task . --slug sample-task",
                        ...
                        "python3 scripts/memory.py profile-render .",
            ...
            _write_text(
                ...
                "\n".join(
                    [
                        ...
                        "python3 scripts/memory.py bootstrap [path] [--full]",
                        ...
                        "python3 scripts/memory.py close-task [path] [--slug <task-slug>] [--strict] [--format text|json]",
                        "python3 scripts/memory.py profile-list",
                        "python3 scripts/memory.py profile-show python-service",
                        "python3 scripts/memory.py profile-apply python-service .",
                        "python3 scripts/memory.py profile-diff python-service .",
                        "python3 scripts/memory.py profile-render [path]",
