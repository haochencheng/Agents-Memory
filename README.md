...
`doctor` 的输出现在按 `Core / Planning / Integration / Optional` 分组，并为每个分组生成健康小结、修复建议、按优先级排序的行动序列，以及带 `Command / Done when` 的 onboarding runbook，更接近 Shared Engineering Brain 的工程操作系统控制台。
